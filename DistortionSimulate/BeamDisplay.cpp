...
IMPLEMENT_DYNAMIC(CBeamDisplay, CDialog)

CBeamDisplay::CBeamDisplay(CWnd* pParent /*=NULL*/)
	: CDialog(CBeamDisplay::IDD, pParent)
{
	m_pImgDataVec.clear();
	m_nPage = 0;
}

CBeamDisplay::~CBeamDisplay()
{
	m_pImgDataVec.clear();
}

void CBeamDisplay::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
}


BEGIN_MESSAGE_MAP(CBeamDisplay, CDialog)
	ON_WM_PAINT()
END_MESSAGE_MAP()
...
BOOL CBeamDisplay::OnInitDialog()
{
...
void CBeamDisplay::SetData(ImgDataVec& DataVec)
{
	m_pImgDataVec = DataVec;
}

void CBeamDisplay::OnPaint()
{
...
LPBITMAPINFO CBeamDisplay::cv_CreatImgInfo(IplImage* pImg)
{
	if(pImg == NULL)
		return NULL;
